...
def get_user_tokens(host):
    user_tokens = SpotifyToken.objects.filter(user=host)
...
def update_or_create_user_tokens(host, access_token, token_type, expires_in, refresh_token):
    ...
    if tokens:
        ...
        tokens = SpotifyToken(user=host, access_token=access_token,
...
def search_function(query, types, limit, host):
    ...
    return execute_spotify_api_request(host, endpoint=f"?q={formatted_query}&type={types}&limit={limit}", url="https://api.spotify.com/v1/search")
