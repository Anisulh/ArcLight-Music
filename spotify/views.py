...
from .utility import execute_spotify_api_request, check_token_if_valid, next_song, pause_song, play_song, prev_song, search_function, update_or_create_user_tokens
...
def IsAuthenticated(request, format=None):
    if request.method == 'POST':
        ...
        is_authenticated = check_token_if_valid(
...
def CurrentlyPlaying(request, format=None):
    ...
    album_cover = item.get('album').get('images')[0].get(
        'url')  # album > images > first image > its url
...
def NextSong(request, format=None):
    ...
    if guest_id == room.host_id or len(votes) + 1 >= votes_needed:
        ...
        vote = Vote(user=guest_id,
                    room=room, song_id=room.currently_playing)
        vote.save()
